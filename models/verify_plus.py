...
def get_features(filename):
    encoding_list1, face_boxes1, face_list = verify1.get_features(filename)
    encoding_list2, _, _ = verify2.get_features(filename)

    encoding_list = []
    for i in range(len(face_boxes1)):
        encoding_list.append(np.concatenate((encoding_list1[i], encoding_list2[i]), axis=0))

    return encoding_list, face_boxes1, face_list
